...
local T1UIColor = {
	...
	["Title Text Color"] = Color3.fromRGB(176, 28, 77),
	...
	["Background Image"] = "",
	["Page Selected Color"] = Color3.fromRGB(176, 28, 77),
	["Section Text Color"] = Color3.fromRGB(220, 220, 210),
	["Section Underline Color"] = Color3.fromRGB(176, 28, 77),
	["Toggle Border Color"] = Color3.fromRGB(176, 28, 77),
	["Toggle Checked Color"] = Color3.fromRGB(230, 230, 230),
	["Toggle Desc Color"] = Color3.fromRGB(185, 185, 185),
	["Button Color"] = Color3.fromRGB(176, 28, 77),
	["Label Color"] = Color3.fromRGB(38, 38, 42),
	["Dropdown Icon Color"] = Color3.fromRGB(230, 230, 230),
	["Dropdown Selected Color"] = Color3.fromRGB(176, 28, 77),
	["Dropdown Selected Check Color"] = Color3.fromRGB(0, 110, 190), 
	["Textbox Highlight Color"] = Color3.fromRGB(176, 28, 775),
	["Box Highlight Color"] = Color3.fromRGB(176, 28, 77),
	["Slider Line Color"] = Color3.fromRGB(176, 28, 77),
	["Slider Highlight Color"] = Color3.fromRGB(0, 110, 190),
...
Library.ToggleUI = function()
	...
	-- Toggle the actual ScreenGui created by this library.
...
local libCreateNoti = function(Setting)
    ...
    TextLabelNoti.Text = "<font color=\"rgb(" .. tostring(color or "255,206,27") .. ")\">" .. tostring("RealKid Hub") .. "</font> " .. tostring(getgenv().TitleNameNoti or "")
...
function Library:Notify(Setting, bypass)
	if getgenv().Config or bypass then
...
function Library:CreateWindow(Setting)
    local TitleNameMain = Setting.Title or "RealKid Hub"
	...
	-- Keep the same layout while rendering it at a device-appropriate size.
	local MainScale = Instance.new("UIScale")
	MainScale.Name = "DeviceScale"
	MainScale.Parent = Main

	local isMobile = uis.TouchEnabled and not uis.KeyboardEnabled
	local preferredScale = isMobile and (Setting.MobileScale or 0.85) or (Setting.DesktopScale or 1.25)
	local viewportConnection
	local function toLayoutPixels(value)
		return value / math.max(MainScale.Scale, 0.001)
	end

	local function updateMainScale()
		local camera = workspace.CurrentCamera
		if not camera then
			MainScale.Scale = preferredScale
			return
		end

		local viewport = camera.ViewportSize
		local fitScale = math.min((viewport.X - 24) / 629, (viewport.Y - 24) / 359)
		MainScale.Scale = math.max(0.45, math.min(preferredScale, fitScale))
	end
	...
	local function watchViewport()
		...
		local camera = workspace.CurrentCamera
		if camera then
			viewportConnection = camera:GetPropertyChangedSignal("ViewportSize"):Connect(updateMainScale)
		end
		updateMainScale()
	end
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
    TextLabelMain.Text = "<font color=\"rgb(" .. tostring(color or "255,206,27") .. ")\">" .. tostring(TitleNameMain or "RealKid Hub") .. "</font> " .. tostring(getgenv().MainDesc or "")
	...
	UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		ControlList.CanvasSize = UDim2.new(0, 0, 0, toLayoutPixels(UIListLayout.AbsoluteContentSize.Y) + 5)
	end)
	...
	function Main_Function:AddTab(PageName)
		...
		Pagelistlayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			PageList.CanvasSize = UDim2.new(0, 0, 0, toLayoutPixels(Pagelistlayout.AbsoluteContentSize.Y))
		end)
		...
		function pageFunction:AddSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			SectionList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
				local contentHeight = toLayoutPixels(SectionList.AbsoluteContentSize.Y) + 5
            ...
            function sectionFunction:AddButton(Setting, Callback)
				...
				Button.Size = UDim2.new(1, 0, 0, Desc and 55 or 40)
				...
				TextColor_1.Position = Desc and UDim2.new(0, 12, 0, 4) or UDim2.new(0, 12, 0, 0)
				TextColor_1.Size = Desc and UDim2.new(1, -110, 0, 25) or UDim2.new(1, -110, 1, 0)
				TextColor_1.Font = Enum.Font.GothamBlack
				TextColor_1.Text = Title
				TextColor_1.TextColor3 = getgenv().UIColor["GUI Text Color"]
				TextColor_1.TextSize = 14
				TextColor_1.RichText = true
				TextColor_1.TextXAlignment = Enum.TextXAlignment.Left

				if Desc then
					local TextColor_Desc = Instance.new("TextLabel")
					TextColor_Desc.Name = "TextColorDesc"
					TextColor_Desc.Parent = RowBG_1
					TextColor_Desc.BackgroundColor3 = Color3.fromRGB(163,162,165)
					TextColor_Desc.BackgroundTransparency = 1
					TextColor_Desc.Position = UDim2.new(0, 12, 0, 24)
					TextColor_Desc.Size = UDim2.new(1, -110, 0, 20)
					TextColor_Desc.Font = Enum.Font.GothamBlack
					TextColor_Desc.Text = Desc
					TextColor_Desc.TextColor3 = getgenv().UIColor["Toggle Desc Color"] or Color3.fromRGB(185, 185, 185)
					TextColor_Desc.TextSize = 13.000
					TextColor_Desc.RichText = true
					TextColor_Desc.TextWrapped = true
					TextColor_Desc.TextXAlignment = Enum.TextXAlignment.Left
				end
				 
				 ClickArea_1.Name = "ClickArea"
				 ClickArea_1.Parent = RowBG_1
				 ClickArea_1.AnchorPoint = Vector2.new(1, 0.5)
				 ClickArea_1.BackgroundColor3 = getgenv().UIColor["Button Color"] or Color3.fromRGB(0, 170, 255)
				 ...
				 UIGradient_1.Color = ColorSequence.new{
					 ColorSequenceKeypoint.new(0, Color3.fromRGB(77, 196, 255)), 
					 ColorSequenceKeypoint.new(0.4, Color3.fromRGB(0, 170, 255)), 
					 ColorSequenceKeypoint.new(0.6, Color3.fromRGB(0, 140, 220)), 
					 ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 110, 190))
				 }
            ...
            function sectionFunction:AddDropdownSection(Setting)
				...
				ScrollContainerList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
					DropdownScroll.CanvasSize = UDim2.new(0, 0, 0, toLayoutPixels(ScrollContainerList.AbsoluteContentSize.Y) + 15)
				end)
				
				InternalList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
					local contentHeight = math.min(toLayoutPixels(InternalList.AbsoluteContentSize.Y) + 10, 300)
			...
			function sectionFunction:AddDropdown(idk, Setting)
				...
				local Default = (function ()
                    if Setting.Default then
                        ...
                        elseif type(Setting.Default) == "table" then
                            return Setting.Default 
                        end
                    end
                    return nil
                ...
                if Selected then
                    ...
                    for _, value in ipairs(List) do
                        if type(Default) == "table" then
                            ListNew[value] = (table.find(Default, value) ~= nil)
                        else
				...
				function dropdownFunction:GetNewList(List)
					...
					local oldSelections = ListNew
					if Selected then
						ListNew = {}
						OrderedList = {}
						for _, value in ipairs(List or {}) do
							ListNew[value] = oldSelections[value] == true
							table.insert(OrderedList, value)
						end
					else
						ListNew = List or {}
...
-- Standalone mode: this file can be executed by itself and will create
-- a visible default window instead of showing only the floating logo.
if not getgenv().BananaUIAutoWindow then
	getgenv().BananaUIAutoWindow = true
